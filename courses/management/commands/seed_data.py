...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Si les données existent déjà, on saute
        try:
            from users.models import User
            if User.objects.filter(username='prof_marie').exists():
                self.stdout.write(self.style.SUCCESS('✅  Données de démo déjà présentes, rien à faire.'))
                return
        except Exception:
            pass  # table pas encore créée, on continue
